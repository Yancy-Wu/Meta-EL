'''
    ...
    train/test.json: LAN ~ MENTION ~ ID
'''
import random
import warnings
...
class GeneralCrosselDataset(CrosselDataset):
    '''
        using zel prediction model to generate train and val task.
        `root`: GeneralCrosselDataset dir location.
    ...
    # candidate using text
    CANDIDATE_USING_TEXT = True

    # document DataFrame: ID - TEXT - TITLE
    _kb: pandas.DataFrame = None
    ...
    def __init__(self, root: str, predictor: Predictor, conf=None):
        ...
        # set function.
        self._generate_way = self._way_2shot if self.CANDIDATE_USING_TEXT else self._way_1shot
        ...
        self._kb = pandas.read_json(f'{root}/kb.json', **kargs).set_index('ID', drop=False)
    ...
    def _way_2shot(self, doc: pandas.Series) -> Way:
        # using text and title as shots.
        return Way([doc['TEXT'], doc['TITLE']], doc['ID'])
    
    # pylint: disable=arguments-differ
    def _way_1shot(self, doc: pandas.Series) -> Way:
        # using text and title as shots.
        return Way([doc['TITLE']], doc['ID'])
    ...
    def _create_task(self, pd: pandas.DataFrame, retain=False) -> List[Task]:
        ...
        for i in progress:
            ...
            # generate task
            candidate_docs = self._kb.loc[candidate_doc_ids]
    ...
    def train_data(self) -> List[Task]:
        return self._create_task(self._train, False)

    def valid_data(self) -> List[Task]:
        return self._create_task(self._valid, True)
